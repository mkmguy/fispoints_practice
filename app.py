...
def event_label(row: pd.Series) -> str:
    return f"{row.get('date', '')} | {row.get('location', '')} | {row.get('discipline', '')} | {row.get('gender', '')} | {row.get('category', '')} | {row.get('result_status', '')}"
...
if run:
    try:
        ...
        m3.metric("Winner's time", f"{format_m_ss_ms(winners_time)}")
